  ...
  const saveChanges = () => {
    const formData = {
      ...
      education: Array.from(
        ...
        const eduElement = eduDiv as HTMLElement; // Cast to HTMLElement
        return {
      ...
      certification: Array.from(
        ...
        const cerElement = cerDiv as HTMLElement; // Cast to HTMLElement
        return {
      ...
      skills: Array.from(document.getElementById('skills')?.children || []).map(
        (skillLi) => {
          const skillElement = skillLi as HTMLElement; // Cast to HTMLElement
      ...
      languages: Array.from(
        ...
        const languageElement = languageLi as HTMLElement; // Cast to HTMLElement
        return languageElement.innerText;
      }),
    };

    // Update the resume preview with the updated formData
